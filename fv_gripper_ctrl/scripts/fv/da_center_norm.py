...
def Get(fvg, fv_data):
  #da_center_norm= np.mean(np.linalg.norm(fv_data.d_obj_center_filtered,axis=1))
  da_center_norm= np.mean([np.linalg.norm(fv_data.d_obj_center_filtered[RIGHT]) if is_detected_r.Get(fvg,fv_data) else 0.0,
                           np.linalg.norm(fv_data.d_obj_center_filtered[LEFT])  if is_detected_l.Get(fvg,fv_data) else 0.0])
  return da_center_norm
